...
import com.digithink.zsretail.repository.PaymentChangeLogRepository;
...
public class SalesHeaderService extends _BaseService<SalesHeader, Long> {
	...
	@Autowired
	private com.digithink.zsretail.service.ReturnVoucherService returnVoucherService;

	@Autowired
	private com.digithink.zsretail.erp.service.SessionExportService sessionExportService;
	...
	public org.springframework.data.domain.Page<SalesHeader> getTicketsHistory(
		...
		// Parse sync status
		com.digithink.zsretail.model.enumeration.SynchronizationStatus syncStatus = null;
		if (syncStatusStr != null && !syncStatusStr.trim().isEmpty() && !syncStatusStr.equalsIgnoreCase("all")) {
			try {
				syncStatus = com.digithink.zsretail.model.enumeration.SynchronizationStatus
		...
		final com.digithink.zsretail.model.enumeration.SynchronizationStatus finalSyncStatus = syncStatus;
		...
		org.springframework.data.jpa.domain.Specification<SalesHeader> spec = (root, query, criteriaBuilder) -> {
			...
			if (finalPaymentMethodId != null) {
				...
				javax.persistence.criteria.Root<com.digithink.zsretail.model.Payment> paymentRoot = paymentSubquery
						.from(com.digithink.zsretail.model.Payment.class);
			...
			// Search filter (by sales number, customer name or ERP document number)
			if (finalSearchStr != null && !finalSearchStr.trim().isEmpty()) {
				...
				javax.persistence.criteria.Predicate erpNoPredicate = criteriaBuilder
						.like(criteriaBuilder.lower(root.get("erpNo")), searchPattern);
				predicates.add(criteriaBuilder.or(salesNumberPredicate, customerNamePredicate, erpNoPredicate));
			}
			...
			if (finalFamilyId != null) {
				...
				javax.persistence.criteria.Root<com.digithink.zsretail.model.SalesLine> lineRoot = familySubquery.from(com.digithink.zsretail.model.SalesLine.class);
			...
			if (finalSubFamilyId != null) {
				...
				javax.persistence.criteria.Root<com.digithink.zsretail.model.SalesLine> lineRoot2 = subFamilySubquery.from(com.digithink.zsretail.model.SalesLine.class);
	...
	public SalesHeader prepareInvoice(Long ticketId, String fiscalRegistration, String invoiceCustomerName) {
		...
		// NAV limits Bill_to_Name_2 (invoice customer name) to 50 characters
		if (invoiceCustomerName != null && invoiceCustomerName.trim().length() > 50) {
			throw new IllegalArgumentException(
					"Invoice customer name must not exceed 50 characters (ERP limit). Current length: "
							+ invoiceCustomerName.trim().length());
		}
